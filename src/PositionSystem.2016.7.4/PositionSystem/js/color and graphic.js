...
function vector(x,y){
	...
	if(arguments.length===4){
		...
		if(x===0&&y===0){
			...
			c.lineCap = "round";
			c.beginPath();
			c.moveTo(constant.x0,constant.y0);
			c.lineTo(constant.x0,constant.y0);
			c.closePath();
			c.stroke();
		}
		else{
...
function power(a){
	...
	point.x = x;
	point.y = y;
	for(x1 = -x; x1 < ca.width - 45; x1 += 0.1){//x1+=0.1;
		y1 = Math.pow(x1, a);
		x = translateX(x1);
		y = translateY(y1);
		c.beginPath();
...
/*function sin(a, omega, phi){
	...
	for(x = constant.x0; x <= canvas.width - 45; x += (0.1 / a)){
		//(0.1 / a):使取点的距离随振幅的变大而减小，使图像在振幅很大的时候仍保持一定精度
		y = translateY(a * Math.sin(omega * x + phi));
		c.beginPath();
		c.moveTo(x, y);
